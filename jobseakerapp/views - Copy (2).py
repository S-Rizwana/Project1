...
def jobseaker_job_details_page(req):
    user_id = 21  # Hardcoded for testing, replace with req.session.get('user_id')
    
    try:
        user = User.objects.get(user_id=user_id)  # Fetch user (ensure the model exists)
        url_link = URL.objects.filter(user_url=user_id).order_by('-url_id')[:1]  # Fetch latest URL
        
        if not url_link.exists():
            messages.warning(req, 'No URL found for this user.')
            return redirect('jobseaker_analyze_job_post')
        
        url = url_link[0].url.strip().rstrip("\\")  # Clean URL
